...
const mutations = {
  ...
  // 最终计算出来的异步路由
  SET_RESULTASYNCROUTES:(state,newAsyncRoutes)=>{
    // vuex保存当前用户的异步路由
    state.resultAsyncRoutes = newAsyncRoutes;
    // 计算出当前用户要展示的所有路由
...
const computedAsyncRoutes = (asyncRoutes,routes)=>{
  // 过滤出当前用户需要展示的异步【超级管理|普通员工】需要展示的异步路由
...
const actions = {
  ...
  async login({ commit }, userInfo) {
    // 解构出用户名与密码（当时没有async和await是因为当时这个东西还没有）
    const { username, password } = userInfo;
    let result = await login({ username: username.trim(), password: password })
    // 注意：当前这个登录的请求现在使用的是mock数据，mock数据code是20000
  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo(state.token).then(response => {
        // 获取用户信息：返回数据包括：用户名，用户头像，routes[返回的标志：不同的用户应该展示哪些菜单的标记]，roles（用户角色）
        // button是按钮的信息，是按钮的权限
        const { data } = response;
        // vuex存储用户的全部信息
